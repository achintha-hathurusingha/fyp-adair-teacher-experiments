"""TEST17: training loop for Models A/N/F2/NF2 x seeds {0,1,2} (12 runs).
Reuses TEST07-B's dataset and teacher-embedding cache READ-ONLY -- exactly
TEST12's dataset/split/embeddings, per TEST17's explicit "do not regenerate"
rule. Training setup (epochs, optimizer, LR, batch, loss) is identical to
TEST12's, per TEST17's fairness rule -- only architecture (norm_type /
conditioning presence) differs between the 4 models.

Saves BOTH a best-validation-PSNR checkpoint and a final-epoch checkpoint
(TEST17 Phase 1's explicit checkpoint-selection requirement).

Usage (on devon, adair-distill env, PINNED):
  taskset -c 0-7,12-31 env OMP_NUM_THREADS=3 MKL_NUM_THREADS=3 python train.py --model A --seed 0
  ... (12 total: A/N/F2/NF2 x 0/1/2)
"""
...
sys.path.insert(0, str(TEST17 / "scripts"))
from models import MODELS, PCA_DIM, CONDITIONED_MODELS  # noqa: E402
...
RESULTS_CSV = TEST17 / "results" / "epoch_metrics.csv"
SEED_SUMMARY_CSV = TEST17 / "results" / "seed_summary.csv"
...
def estimate_macs(model, input_shape=(1, 3, 128, 128), device="cuda"):
    try:
        fyp = TEACHER_EXP.parent / "fyp-adair-distill"
        sys.path.insert(0, str(fyp))
...
def main():
    ...
    use_kd = args.model in CONDITIONED_MODELS

    CKPT_DIR.mkdir(parents=True, exist_ok=True)
    best_psnr_so_far = -1.0
    epoch_rows = []
    t_start = time.time()
    for epoch in range(1, EPOCHS + 1):
        ...
        if overall_psnr > best_psnr_so_far:
            best_psnr_so_far = overall_psnr
            torch.save(model.state_dict(), CKPT_DIR / f"model_{args.model}_seed{args.seed}_best.pt")
    ...
    torch.save(model.state_dict(), CKPT_DIR / f"model_{args.model}_seed{args.seed}_final.pt")
    ...
    print(f"[{args.model}/s{args.seed}] DONE. last5_mean_psnr={summary_row['last5_mean_psnr']:.3f} "
          f"last5_mean_ssim={summary_row['last5_mean_ssim']:.4f} total_time={time.time()-t_start:.0f}s", flush=True)
