"""TEST17 Phase 4: grouped degradation-classification probe + teacher
cosine alignment for F2's e_D vs NF2's e_D -- reused pattern from
TEST12/representation_probe.py. Uses each seed's BEST-validation-PSNR
checkpoint (the canonical deployed checkpoint for this experiment).

Usage (devon, adair-distill env, PINNED):
...
OUT_DIR = TEST17 / "results" / "statistics"
...
def main():
    ...
    for model_name in KD_MODELS:
        for seed in SEEDS:
            ckpt_path = CKPT_DIR / f"model_{model_name}_seed{seed}_best.pt"
            if not ckpt_path.exists():
                print(f"SKIP {model_name}/seed{seed}: checkpoint not found ({ckpt_path})")
                continue
    ...
    print("\n=== Teacher cosine alignment by model (mean across seeds) ===")
    print(align_df.groupby("model")["mean_cosine_similarity"].mean().to_string())
