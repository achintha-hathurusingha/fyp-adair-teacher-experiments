"""TEST08-C: primary statistical analysis. Per-seed paired deltas for
C-A, C-B, B-A (last5-window PSNR/SSIM primary metric, best-epoch secondary),
mean+-std across the 3 seeds, bootstrap CI (exploratory, N=3), and
per-degradation (Rain/Haze/Noise) breakdown for all three comparisons.

The most important comparison is C-B (does conditioning help beyond simple
KD, which already achieves representation matching).
...
def main():
    ...
    per_model = {m: df[df.model == m].set_index("seed").sort_index() for m in ["A", "B", "C"]}
    for m in ["A", "B", "C"]:
