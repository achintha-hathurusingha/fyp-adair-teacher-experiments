"""TEST08-C: training loop for Models A/B/C x seeds {0,1,2} (9 runs).
Identical config across all three except the KD/conditioning branches.
Reuses TEST07-B's dataset and teacher-embedding cache READ-ONLY (same
scene split, same crops, same leakage-safe PCA-16 fit) -- verified
identical, so no new dataset/PCA fit is needed per the task spec.

Each run writes its OWN per-run CSV files (no shared-file writes during
parallel execution) -- this pattern was required after TEST07-B hit a
data-corrupting race condition with 6-way concurrent shared-CSV writes;
merge_epoch_metrics.py combines them afterward.

Usage (on devon, adair-distill env, PINNED):
  taskset -c 0-7,12-31 env OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 python train.py --model A --seed 0
  ... (9 total: A/B/C x 0/1/2)
"""
...
# READ-ONLY reuse of TEST07-B's dataset manifest + teacher embedding cache
# (same scene split, same crops, same leakage-safe PCA-16 fit -- verified
# identical, per the task's "reuse only if exact split/crop definition are
# identical" rule). TEST07-B's directory is never written to.
...
RESULTS_CSV = TEST08C / "results" / "epoch_metrics.csv"
SEED_SUMMARY_CSV = TEST08C / "results" / "seed_summary.csv"
...
def main():
    ...
    # Per-run files ONLY -- no shared-file writes during parallel execution.
    # merge_epoch_metrics.py combines them into epoch_metrics.csv/seed_summary.csv
    # in a single-threaded step afterward.
