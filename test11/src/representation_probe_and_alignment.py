"""TEST11: grouped degradation-classification probe for teacher PCA-16 and
each model's e_S (A has none), plus teacher-student embedding alignment
(cosine, MSE) for all F-variants across ranks.

Usage (on devon, adair-distill env, PINNED):
  taskset -c 0-7,12-31 python representation_probe_and_alignment.py
"""
...
def main():
    ...
    probe_rows.append({"representation": "teacher_PCA16", "model": "teacher", "rank": None, "seed": None,
    ...
    for model_name in F_MODELS:
        ...
        for seed in SEEDS:
            ...
            probe_rows.append({"representation": f"model_{model_name}_eS", "model": model_name, "rank": rank,
                                "seed": seed, "accuracy": acc, "balanced_accuracy": bal_acc, "macro_f1": f1})
            align_rows.append({"model": model_name, "rank": rank, "seed": seed,
                                "mean_cosine_similarity": float(np.mean(cos_sims)),
                                "mean_mse": float(np.mean(mse_vals))})
            print(f"{model_name}(R={rank})/seed{seed}: probe_acc={acc*100:.1f}% "
                  f"cosine={np.mean(cos_sims):.4f} mse={np.mean(mse_vals):.4f}", flush=True)
    ...
    print(f"wrote {OUT_DIR / 'teacher_alignment.csv'}")

    print("\n=== Probe accuracy by rank (mean across seeds) ===")
    print(probe_df[probe_df.model != "teacher"].groupby("model")["accuracy"].mean().to_string())
    print("\n=== Teacher alignment by rank (mean across seeds) ===")
    print(align_df.groupby("model")[["mean_cosine_similarity", "mean_mse"]].mean().to_string())
