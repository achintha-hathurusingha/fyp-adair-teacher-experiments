"""TEST11: training loop for Models A/F2/F4/F8/F16 x seeds {0,1,2} (15 runs).
Identical config across all models except rank R (F-variants only). Reuses
TEST07-B's dataset and teacher-embedding cache READ-ONLY (same scene split,
crops, leakage-safe PCA-16 fit as TEST07-B/08-C/09/10/10-R).

Each run writes its OWN per-run CSV files (no shared-file writes during
parallel execution) -- established pattern since TEST07-B's shared-CSV race.

Usage (on devon, adair-distill env, PINNED):
  taskset -c 0-7,12-31 env OMP_NUM_THREADS=3 MKL_NUM_THREADS=3 python train.py --model A --seed 0
  ... (15 total: A/F2/F4/F8/F16 x 0/1/2)
"""
...
RESULTS_CSV = TEST11 / "results" / "epoch_metrics.csv"
SEED_SUMMARY_CSV = TEST11 / "results" / "seed_summary.csv"
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        model.train()
        train_losses, kd_losses, coeff_mags, grad_norms = [], [], [], []
        for degraded, clean, e_t, deg, scene_id in train_loader:
            ...
            if use_kd:
                gn = float(sum(p.grad.norm() ** 2 for p in model.parameters() if p.grad is not None) ** 0.5)
        ...
        if epoch % 5 == 0 or epoch == 1:
            gn_str = f"{row['mean_grad_norm']:.3f}" if row["mean_grad_norm"] is not None else "n/a"
            print(f"[{args.model}/s{args.seed}] epoch {epoch}/{EPOCHS}: L1={row['train_l1_loss']:.4f} "
                  f"KD={row['train_kd_loss']:.4f} grad_norm={gn_str} "
