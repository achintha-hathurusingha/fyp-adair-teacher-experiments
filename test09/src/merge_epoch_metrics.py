"""TEST09: merge the 18 per-run epoch_metrics_{model}_seed{seed}.csv and
seed_summary_{model}_seed{seed}.csv files into unified epoch_metrics.csv and
seed_summary.csv.

Usage (on devon, adair-distill env, PINNED, run AFTER all 18 runs finish):
...
def main():
    ...
    print(summary_df[["model", "seed", "final_psnr", "best_psnr", "last5_mean_psnr", "params"]].to_string(index=False))
