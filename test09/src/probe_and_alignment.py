"""TEST09: grouped degradation-classification probe for teacher PCA-16 and
each model A-F's bottleneck (model.bottleneck_pooled()) and e_S (for KD
models B/C/D/E/F, via model(x) -> (out, e_s)), aggregated across seeds.
Also teacher-student cosine/MSE alignment for B/C/D/E/F.
        ...
        for seed in SEEDS:
            ...
            if model_name in KD_MODELS:
                ...
                align_rows.append({"model": model_name, "seed": seed,
                                    ...
                                    "mean_mse": float(np.mean(mse_vals)), "n_examples": len(cos_sims)})
