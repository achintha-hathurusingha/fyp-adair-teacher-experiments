"""TEST09: primary statistical analysis. Per-seed paired deltas for
D-C, E-C, F-C, C-B, B-A (last5-window PSNR/SSIM primary metric, best-epoch
secondary), mean+-std across 3 seeds, bootstrap CI (exploratory, N=3), and
per-degradation (Rain/Haze/Noise) breakdown for all five comparisons.

D-C / E-C / F-C are the key comparisons: do deeper FiLM (D/E) or low-rank
channel-mixing (F) improve over bottleneck-only FiLM (C), especially Haze?
...
def main():
    ...
    per_model = {m: df[df.model == m].set_index("seed").sort_index() for m in ["A", "B", "C", "D", "E", "F"]}
