"""TEST09: training loop for Models A/B/C/D/E/F x seeds {0,1,2} (18 runs).
Identical config across all six except the KD/conditioning branches.
Reuses TEST07-B's dataset and teacher-embedding cache READ-ONLY (same
scene split, same crops, same leakage-safe PCA-16 fit).

Each run writes its OWN per-run CSV files (no shared-file writes during
parallel execution) -- the pattern required after TEST07-B's shared-CSV
race condition; merge_epoch_metrics.py combines them afterward.

Usage (on devon, adair-distill env, PINNED):
  taskset -c 0-7,12-31 env OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 python train.py --model A --seed 0
  ... (18 total: A/B/C/D/E/F x 0/1/2)
"""
...
RESULTS_CSV = TEST09 / "results" / "epoch_metrics.csv"
SEED_SUMMARY_CSV = TEST09 / "results" / "seed_summary.csv"
