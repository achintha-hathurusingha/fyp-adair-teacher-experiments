"""TEST16 Phase 4-5: compile + profile the 4 complete student graphs on
Qualcomm AI Hub, same device/runtime path as TEST15 (Snapdragon 8 Elite QRD,
--target_runtime qnn_context_binary). Submits then blocks on job.wait() for
each (TEST15's collect_results.py bug fix -- job.wait(), not a manual
.running poll loop, since queued jobs are .pending not .running).

Extracts, per model: compile success, profile success, per-layer compute
unit assignment (NPU/CPU/GPU, so full-graph fallback -- not just isolated
op fallback -- is verified), and the full `all_inference_times` array from
the profile report (Qualcomm AI Hub runs ~100 warm reps per profile job),
from which mean/median/p50/p90/p95/std are computed -- Phase 5's explicit
requirement to not rely on one latency number.
...
JOBS_DIR = TEST16 / "results" / "profile_jobs"
...
def main():
    ...
    with open(TEST16 / "results" / "export_manifest.json") as f:
    ...
    for rec in job_records:
        ...
        if profile_success:
            ...
            n_cpu_layers = sum(1 for d in layer_details if d.get("compute_unit") == "CPU")
            n_gpu_layers = sum(1 for d in layer_details if d.get("compute_unit") == "GPU")
            row["n_cpu_layers"] = n_cpu_layers
