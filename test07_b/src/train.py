"""TEST07-B: training loop for Models A/B x seeds {0,1,2}. Identical config
across A/B except the KD branch. Uses ALL precomputed crops (640 train crops
x 3 degradations = 1920 examples/epoch for train; 20 val crops x 3 = 60,
fixed, deterministic).

Usage (on devon, adair-distill env, PINNED):
  taskset -c 0-7,12-31 python train.py --model A --seed 0
  taskset -c 0-7,12-31 python train.py --model B --seed 0
  ... (6 total: A/B x 0/1/2)
"""
...
CKPT_DIR = TEST07B / "results" / "checkpoints"
RESULTS_CSV = TEST07B / "results" / "epoch_metrics.csv"
SEED_SUMMARY_CSV = TEST07B / "results" / "seed_summary.csv"
...
def main():
    ...
    # This run's own data ONLY -- computed from the in-memory epoch_rows list,
    # NOT from any file re-read/merge. Writing 6 parallel processes to a
    # SHARED CSV caused a data-corrupting race in an earlier run of this
    # script (every run's "final_psnr" ended up identical, pulled from
    # whichever process's data happened to be in the file at merge time).
    # Fix: each run writes its OWN per-run CSV (no shared file, no race);
    # a separate merge step (merge_epoch_metrics.py) combines them afterward.
    ...
    # ---- checkpoint-selection summary (from this_run_df ONLY) ----
    ...
    # Per-run file, same rationale as epoch_metrics above -- avoids the
    # shared-file write race entirely rather than trying to merge safely
    # under concurrency. merge_epoch_metrics.py combines these afterward.
