"""TEST07-B: after all 6 training runs, (1) probe degradation
classification for teacher PCA-16, Model A bottleneck (GAP+GMP), Model B
bottleneck (GAP+GMP), and Model B's compact e_S -- per seed, then
aggregated mean+-std across seeds; (2) teacher-student embedding alignment
(cosine similarity, MSE, per-component MSE) for Model B, per seed.
...
OUT_DIR = TEST07B / "results" / "statistics"
...
def main():
    ...
    # ---- teacher PCA-16 (seed-independent, single teacher) ----
    ...
    for model_name in ["A", "B"]:
        for seed in SEEDS:
            ...
            if model_name == "B":
                ...
                probe_rows.append({"representation": "model_B_compact_eS", "model": "B", "seed": seed,
                                    "dim": X_es.shape[1], "accuracy": acc, "balanced_accuracy": bal_acc,
                                    "macro_f1": f1})
                print(f"model_B_compact_eS seed{seed}: acc={acc*100:.1f}% f1={f1:.3f}", flush=True)

                per_comp = np.stack(per_comp_sq_errs).mean(axis=0)
                align_rows.append({"seed": seed, "mean_cosine_similarity": float(np.mean(cos_sims)),
                                    "mean_mse": float(np.mean(mse_vals)),
                                    "per_component_mse": per_comp.tolist(),
                                    "n_examples": len(cos_sims)})
                print(f"B/seed{seed} alignment: cosine={np.mean(cos_sims):.4f} mse={np.mean(mse_vals):.4f}", flush=True)
