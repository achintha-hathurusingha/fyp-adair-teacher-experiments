"""TEST10: Models A/F/G, built on the exact TEST07-B/08-C/09 locked NAFNet
M-arm, read-only imported from fyp-adair-distill.

  A: baseline, no KD, no conditioning.
  F: TEST09's best mechanism reproduced -- compact latent KD + low-rank
     (rank=2, per this task's spec) channel-mixing conditioning at the
     bottleneck.
  G: F + restoration-trajectory distillation. Adds THREE trainable
     projection heads (P_S^l) that pool+project the student's own
     decoder-stage outputs (decoders[0]/[1]/[2], i.e. the 16x16/32x32/64x64
     stages -- see report/teacher_stage_audit.md for why these three) into
     a shared 32-dim compact space, for comparison against the matching
     teacher AFLB.aflb_out stages (projected by a separate, jointly-trained
     TeacherTrajectoryHeads module in teacher_trajectory.py). These heads
     are TRAINING-ONLY: they read intermediate features off to the side and
     do not affect the restoration output, so they can be discarded at
     inference (see verify_inference_graph.py).
"""
...
PCA_DIM = 16
LOWRANK_R = 2  # per TEST10 spec (TEST09 used rank=4)
TRAJ_DIM = 32
...
class ModelF(PilotNAFNetBase):
    """TEST09's best mechanism: compact latent KD + low-rank (rank=2)
    channel-mixing conditioning at the bottleneck."""
...
class ModelG(ModelF):
    """F + restoration-trajectory distillation. Adds 3 trainable projection
    heads reading the student's own decoder-stage outputs (pre-NAFBlock,
    i.e. immediately after upsample+skip -- the point that structurally
    corresponds to "just received the upstream signal", analogous to the
    teacher's aflb_out being "just finished this depth's transformation").
    These heads are TRAINING-ONLY (see forward_trajectory); forward() is
    identical to Model F's and does not touch them."""
